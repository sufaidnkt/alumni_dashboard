    ...
    public function responseHandler(ResponseInterface $responseObj) {
        ...
        if ($httpResponseCode == 200) {
            if (!$body) {
            	...
            	error_log(getmypid()."::Blocktrail error 200 $caller");
            	$error_bk =serialize($body);
            	error_log("Blocktrail error 200 $error_bk");
            ...
            $caller = debug_backtrace()[1]['function'];
            error_log(getmypid()."::Blocktrail error 400 OR 403 $caller");
            $error_bk =serialize($body);
            error_log("Blocktrail error 400 OR 403 $error_bk");
        	...
        	$caller = debug_backtrace()[1]['function'];
        	error_log(getmypid()."::Blocktrail error 401 $caller");
        	$error_bk =serialize($body);
        	error_log("Blocktrail error 401 $error_bk");
            ...
            if ($httpResponsePhrase == "Endpoint Not Found") {
            	$caller = debug_backtrace()[1]['function'];
            	error_log(getmypid()."::Blocktrail error 404 $caller");
            	$error_bk =serialize($body);
            	error_log("Blocktrail error 404 $error_bk");
        	...
        	$caller = debug_backtrace()[1]['function'];
        	error_log(getmypid()."::Blocktrail error 500 $caller");
        	$error_bk =serialize($body);
        	error_log("Blocktrail error 500 $error_bk");
            throw new GenericServerError(Blocktrail::EXCEPTION_GENERIC_SERVER_ERROR . "\nServer Response: " . $body, $httpResponseCode);
        } else {
        	$caller = debug_backtrace()[1]['function'];
        	error_log(getmypid()."::Blocktrail error else $caller");
        	$error_bk =serialize($body);
        	error_log("Blocktrail error else $error_bk");
